...
export default function RegistrosPage() {
  ...
  const loadRegistros = async () => {
    try {
      setLoading(true)
      const response = await registrosService.list({
        skip: page * limit,
        limit,
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-800 text-white text-xs uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4 font-semibold">Siniestro</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-slate-500">
                      No hay registros. ¡Creá el primero!
                    </td>
                  </tr>
                ) : (
                  registros.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-medium text-slate-700">{r.actor}</div>
                        <div className="text-xs text-indigo-500">{r.oblea || '-'}</div>
                      </td>
                      <td className="px-6 py-4 text-slate-600">{r.expediente}</td>
                      <td className="px-6 py-4 text-slate-600">{r.nSiniestro || '-'}</td>
                      <td className="px-6 py-4">
